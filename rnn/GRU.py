...
class GRU(object):
    ...
    def __init__(self, nh, nc, de, cs, ne_char=None, de_char=None, ne_src=None, 
        ...
        self.train = theano.function( inputs  = [idxs, y, lr],
                                      outputs = nll,
                                      updates = updates,
				      on_unused_input='ignore', name='train_rnn',
...
class GRU_adadelta_char(object):
	...
	normalization_list = [self.emb]
        ...
        #Recurent memory or h(t-1)
        self.h0  = theano.shared(numpy.zeros(nh, dtype=theano.config.floatX))
        self.h0_char  = theano.shared(numpy.zeros(nh, dtype=theano.config.floatX))
        self.prev  = theano.shared(numpy.zeros(nc, dtype=theano.config.floatX))
        # bundle
	...
	self.h0_char, self.prev]
	 ...
	 'bh', 'bh_char', 'br','br_char', 'bu', 'bu_char', 'b', 'b_char', 'h0', 'h0_char', 'prev']
        ...
        [c, ch, s, s_t],_ = theano.scan(fn=recurrence, \
            sequences=[x, x_char], outputs_info=[self.h0, self.h0_char, self.prev, None],
					n_steps=x.shape[0])
...
class GRU_adadelta_char_pretrain(object):
	...
	self.Wh_char, self.Wr, self.Wr_char, self.Wu, self.Wu_char, self.W, \
	self.W_char, self.Ur, self.Ur_char, self.Uu, self.Uu_char, self.bh, \
	self.bh_char, self.br, self.br_char, self.bu, self.bu_char, self.b, self.b_char, 
	self.h0, self.h0_char, self.prev ]
...
class GRU_adadelta_char_bilingual(object):
    def __init__(self, nh, nc, de, cs, ne_char=None, de_char=None, ne_src=None, 
		ne_tgt=None, emb_src=None, emb_tgt=None, max_char=10):
        '''
        nh :: dimension of the hidden layer
	...
	self.h0_char, self.prev ]
	 ...
	 'bh', 'bh_char', 'br','br_char', 'bu', 'bu_char', 'b', 'b_char', 'h0', 'h0_char', 'prev']
        ...
        [c, ch, s, s_t],_ = theano.scan(fn=recurrence, \
            sequences=[x_src, x_tgt, x_char], outputs_info=[self.h0, self.h0_char, self.prev, None],
					n_steps=x_src.shape[0])
