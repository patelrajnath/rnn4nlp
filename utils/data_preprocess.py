...
def preprocess_data(data_train, data_train_y, data_test, data_test_y, data_valid, data_valid_y,
	...
	w2idxs, char2idxs, embs, train, train_y, test, test_y, valid, valid_y = ([] for i in range(9))
	...
	if use_char:
		for tf, char2idx in zip(data_train, char2idxs):
    			train.append(char2index(tf, char2idx))
		for tf, char2idx in zip(data_test, char2idxs):
    			test.append(char2index(tf, char2idx))
		for tf, char2idx in zip(data_valid, char2idxs):
    			valid.append(char2index(tf, char2idx))
	...
	data = [train, train_y, test, test_y, valid, valid_y, w2idxs, char2idxs, label2idx, embs]
