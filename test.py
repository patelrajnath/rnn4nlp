...
select_model = {
		"GRU_adadelta_bilingual_pretrain": GRU_test.GRU_adadelta_bilingual,
		"GRU_adadelta_bilingual": GRU_test.GRU_adadelta_bilingual, 
		"GRU_adadelta_pretrain": GRU_test.GRU_adadelta,
		"GRU_adadelta": GRU_test.GRU_adadelta,
		}
...
def test(data_test=['data/qe/test/test.src.lc',
	...
	test, test_y, w2idxs, label2idxs, embs = processed_data
	idx2label = dict((k,v) for v,k in label2idxs.iteritems())

	vocsize_s = vocsize_t = 0
        emb_s, emb_t, test_s, test_t = ([] for i in range(4))
		
	if model_options['use_bilingual'] or len(test) == 2:
		emb_s, emb_t = embs
		test_s, test_t = test
